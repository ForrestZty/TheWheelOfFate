...
namespace TBTK {
	...
	public class FactionManagerEditorWindow : EditorWindow {
		...
		Vector2 DrawFactionConfigurator(float startX, float startY, Faction faction){
			...
			EditorGUI.ColorField(new Rect(startX+spaceX+width+10, startY, width-70, height), faction.color);
			...
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width+50, height), cont);
			faction.loadFromData=EditorGUI.Toggle(new Rect(startX+spaceX, startY, width, height), faction.loadFromData);
